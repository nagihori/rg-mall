...
export function SiteChrome({ children, settings }: { children: React.ReactNode; settings: SiteSettingsViewModel }) {
  const pathname = usePathname()
  const hideHeader = pathname?.startsWith('/events/') || pathname?.startsWith('/stores/')
  return (
    <>
      {!hideHeader && <SiteHeader imageUrl={settings.headerImageUrl} imageAlt={settings.headerImageAlt} tagline={settings.tagline} />}
      {children}
