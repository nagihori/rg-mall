...
export async function getPublicEvents(): Promise<EventCardViewModel[]> {
  ...
  return results.docs.map((doc) => presentEventCard({ id: String(doc.id), title: doc.title, slug: doc.slug, summary: doc.summary, body: doc.body, startsAt: doc.startsAt, endsAt: doc.endsAt, publishedAt: doc.publishedAt, status: doc.status as 'published' | 'archived', heroImage: typeof doc.heroImage === 'object' && doc.heroImage ? { url: doc.heroImage.url, alt: doc.heroImage.alt } : null })).filter((event): event is EventCardViewModel => event !== null)
}
