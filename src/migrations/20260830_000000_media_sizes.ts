...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE INDEX "media_sizes_thumbnail_filename_idx" ON "media" USING btree ("sizes_thumbnail_filename");
  CREATE INDEX "media_sizes_card_filename_idx" ON "media" USING btree ("sizes_card_filename");`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   DROP INDEX "media_sizes_thumbnail_filename_idx";
  DROP INDEX "media_sizes_card_filename_idx";
