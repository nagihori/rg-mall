...
export const Users: CollectionConfig = { slug: 'users', auth: { disableLocalStrategy: true }, admin: { useAsTitle: 'discordUsername' }, access: { read: isAdmin, create: isAdmin, update: isAdmin, delete: isAdmin }, hooks: { afterChange: [async ({ doc, previousDoc, req }) => { if (previousDoc && doc.role !== previousDoc.role) await recordRoleChange(req, doc.id); return doc }] }, fields: [
